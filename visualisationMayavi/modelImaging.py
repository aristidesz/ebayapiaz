...
# 		ynew = np.linspace(-33, 33, 66)
# 		xnew = np.linspace(-50, 50, 100)
# 		z = m1[:,:,i]
...
# src = mlab.pipeline.scalar_field(m2)
# mlab.pipeline.image_plane_widget(src,plane_orientation='z_axes',slice_index=10,colormap='RdBu',vmin=-0.4,vmax=0.4)
...
bmap=Basemap(projection='merc',llcrnrlat=-30, llcrnrlon=75,urcrnrlat=30, urcrnrlon=165,lat_0=0,lon_0=float((75+165)/2),ax=ax1,resolution='f')
bmap.bluemarble(ax = ax1)
# bmap.drawcoastlines()

bmdata = bmap.etopo(ax = ax1)._A.data
# mlab.imshow(bmdata[:,:,2],colormap='gist_earth',opacity=0.8)

try:
	# 
	nx,ny,nz = bmdata.shape
	x = np.linspace(0, ny, ny)
	y = np.linspace(0, nx, nx)
	xnew = np.linspace(0, ny, 100)
	ynew = np.linspace(0, nx, 66)
	z = bmdata[:,:,0]

	f = interpolate.interp2d(x, y, z, kind='cubic')
	znew = f(xnew, ynew)
	obj = mlab.imshow(znew,opacity=1,vmin=0.5,vmax=1.5)
	obj.actor.position = [33, 50, 0]     # the required  position
	import pdb;pdb.set_trace()
except Exception as e:
	print e
	import pdb;pdb.set_trace()
...
try:
	...
	znew = f(xnew, ynew)
	azsurf = mlab.surf(znew,extent=[0,66,0,100,0,11])
except Exception as e:
	print e
	import pdb;pdb.set_trace()

mlab.axes(xlabel='Latitude', ylabel='Longitude', zlabel='Depth',ranges=(-30,30,75,165,10,800),nb_labels=5,extent=[0,66,0,100,0,20])
