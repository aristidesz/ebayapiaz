	...
	for file in filenames:
		...
		with open(mypath+file,'r+') as fread:
			...
			try:
				...
				uncertainty_lst = [] 
				for i,(myl,myi) in enumerate(zip(labels,myreal)):
					uncertainty = round(lbStd[myl],2)
					if np.isinf(uncertainty):
						uncertainty = 0.1
					uncertainty_lst.append(uncertainty)
				data = uncertainty_lst
				scaler = MinMaxScaler(feature_range=(0.1, 0.5))
				scaler.fit(data)
				...
				with open('./picks/'+file,'w') as fw:
					for myl,myi in zip(labels,myreal):
						...
						if np.isinf(uncertainty):
							uncertainty = 0.1
						uncertainty =str(round(scaler.transform([uncertainty])[0],2))
					# uncertainty = '0.1'		
