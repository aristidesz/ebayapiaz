...
for (dirpath, dirnames, filenames) in walk(mypath):
	for file in filenames:
		...
		with open(mypath+file,'r+') as fread:
			...
			# import pdb;pdb.set_trace()
			myfeatures = [x1,y1,z1]
			X = np.array(zip(x1,y1,z1))
			# X = np.transpose(myfeatures)
			...
			try:
				...
				# import pdb;pdb.set_trace()
				db = DBSCAN(eps=0.00680001, min_samples=2).fit(X)
				labels = db.labels_
				...
				newFeatures = np.array(zip(x1,y1,z1,scaler,labels))
				nbrs = NearestNeighbors(n_neighbors=n_clusters_, algorithm='ball_tree').fit(newFeatures)
				distances, indices = nbrs.kneighbors(newFeatures)
				mean = np.mean(distances)
				print "Mean distance= ",mean
				db = DBSCAN(eps=0.00680001, min_samples=2).fit(newFeatures)
				labels = db.labels_
				mylabels = []
				...
				for elem in labdict.items():
					...
					if elem[0] != -1:
						if wi == 0:
							weight = 0.1
							# lbStd[elem[0]] = np.std(labelTT)*1/100
						else:
							weight = np.divide(1, wi)
							# import pdb;pdb.set_trace()
					else:
						weight = 0.1
					lbStd[elem[0]] = weight
				...
				with open('./picks/'+file,'w') as fw:
					for myl,myi in zip(labels,myreal):
						if myl == -1:
							uncertainty = '0.1'
							# print myl,uncertainty
						else:
							# print myl,round(lbStd[myl],2)
